...
def separate_data(fovs, img_size):
    ...
    for tiff in fovs:
        ...
        bright_field.append(image3d_prep(img_3d))

        img_3d = img[n_channels - 4, under_slice:upper_slice, :, :]
        fluorescent.append(image3d_prep(img_3d))
...
def image3d_prep(img_3d: np.ndarray):
    z, y, x = img_3d.shape
    img_3d_padded = np.zeros((z, y + 16, x + 100))
    for i in range(z):
        img_3d_padded[i] = cv2.copyMakeBorder(img_3d[i], 0, 16, 0, 100, cv2.BORDER_REPLICATE)
