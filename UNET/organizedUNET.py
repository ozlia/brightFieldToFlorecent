...
class SpecialUNet(iUNet):
    ...
    def get_up_sample_layer(self, layer_input=None, skip_input=None, filters=64, f_size=(3, 3), strides=(2, 2),
        ...
        upsample_layer = concatenate([layer_input, skip_input])
        upsample_layer = Conv2DTranspose(filters, f_size, padding='same',activation=LeakyReLU())(upsample_layer)
        if dropout_rate > 0:
            upsample_layer = Dropout(dropout_rate)(upsample_layer)
        upsample_layer = Conv2DTranspose(filters, f_size, strides=2, padding='same',activation=LeakyReLU())(upsample_layer)
        return upsample_layer

    def build_model(self, input_dim, base_filters, filter_size, strides):
        inputs = Input(shape=input_dim)
        # encoder
        c1, d1 = self.get_down_sample_layer(inputs, base_filters, filter_size, strides)
        c2, d2 = self.get_down_sample_layer(d1, base_filters * 2, filter_size, strides)
        c3, d3 = self.get_down_sample_layer(d2, base_filters * 4, filter_size, strides)
        c4, d4 = self.get_down_sample_layer(d3, base_filters * 8, filter_size, strides)

        # embedding
        d4 = Conv2D(base_filters * 16, filter_size, activation='relu', padding='same')(d4)
        d5 = Conv2DTranspose(base_filters * 16, filter_size, strides=strides, activation='relu', padding='same')(d4)
        ...
        model.compile(optimizer='adam', loss='mse')  # optimizer = Adam(0.0002, 0.5)
        return model
