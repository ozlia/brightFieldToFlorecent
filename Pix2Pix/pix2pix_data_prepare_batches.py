...
class pix2pix_data_prepare():
    def __init__(self, batch_size=128):
        ...
        self.brightfield_train_paths = os.listdir(self.patches_batches_dirs_paths[f'brightfield_train'])
        self.fluorescent_train_paths = os.listdir(self.patches_batches_dirs_paths[f'fluorescent_train'])

    def load_images_as_batches(self, sample_size=-1):
        ...
        if sample_size != -1:
            ...
            for brightfield_batch_path, fluorescent_batch_path in zip(self.brightfield_train_paths,
                ...
                brightfield_batch = utils.load_numpy_array(brightfield_batch_path)
                fluorescent_batch = utils.load_numpy_array(fluorescent_batch_path)
        ...
        try:
            for b_f in ['brightfield', 'fluorescent']:
                for test_train in ['test', 'train']:
                    target_path = os.path.join(DIRECTORY, self.org_type[:-1], test_train, str(self.batch_size), b_f)
